...
class RegresserModel(nn.Module):
    ...
    def forward(self, imgs, edges_f, edges, meshes, vs):
        out_mask, out_map = self.seg_net(imgs)
        edges_map = add_feature(edges, out_map, vs)
        edges_input = torch.cat([edges_f, edges_map], dim=1)
